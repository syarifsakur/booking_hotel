    ...
    <form action="{{ route('admin.rooms.update', $room->id) }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="mb-6">
            ...
            <select id="type" name="type" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 @error('type') border-red-500 @enderror" required>
                ...
                <option value="Twin" @selected(old('type', $room->type) == 'Twin')>Twin Room</option>
                <option value="Suite" @selected(old('type', $room->type) == 'Suite')>Suite Room</option>
                <option value="Family" @selected(old('type', $room->type) == 'Family')>Family Room</option>
            </select>
            @error('type')
                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
            @enderror
